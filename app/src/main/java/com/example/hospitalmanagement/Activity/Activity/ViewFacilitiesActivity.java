...
public class ViewFacilitiesActivity extends AppCompatActivity {
    private List<Facility> facilityList=new ArrayList<>();
    private RecyclerView recyclerView;
    private FacilityAdpter mAdapter;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.v("hos_id",hosid);
        mAdapter = new FacilityAdpter(facilityList);
        recyclerView = (RecyclerView) findViewById(R.id.facility_recyclerview);
        RecyclerView.LayoutManager mLayoutManager = new LinearLayoutManager(getApplicationContext());
        recyclerView.setLayoutManager(mLayoutManager);
        recyclerView.setItemAnimator(new DefaultItemAnimator());
        recyclerView.addItemDecoration(new DividerItemDecoration(this, 0));
        recyclerView.setAdapter(mAdapter);
        RequestQueue requestQueue= Volley.newRequestQueue(ViewFacilitiesActivity.this);
        String Url="http://hospitalmanagement.fabstudioz.com/viewfacility.php";
            ...
            public void onResponse(String response) {
                try {
                    ...
                    for (int i=0;i<=jsonArray.length();i++)
                    {
                        ...
                        Facility facility=new Facility(caption,description,image,context);
